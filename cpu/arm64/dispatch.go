...
func (fc *funcCompiler) dispatchOp(r *decode.Reader, op byte) error {
	...
	case 0x04: // if
		...
		fc.emitPopR(X9)
		fc.CMPI32(X9, 0)
		elseOff := fc.BCond(CondEQ) // b.eq .else
		fc.ctrl = append(fc.ctrl, ctrlFrame{
	...
	case 0x05: // else
		...
		fc.PatchCondImm19(top.elseJmpOff, fc.Pos())
		top.elseJmpOff = 0
	...
	case 0x0B: // end
		...
		if frame.kind == ctrlIf && frame.elseJmpOff != 0 {
			fc.PatchCondImm19(frame.elseJmpOff, fc.Pos())
		}
		for _, p := range frame.endPatches {
			fc.PatchBranchImm26(p, fc.Pos())
		}
		if len(fc.ctrl) == 0 {
			fc.emitEpilogue()
			fc.dead = true
	...
	case 0x0D: // br_if
		...
		fc.emitPopR(X9)
		fc.CMPI32(X9, 0)
		p := fc.BCond(CondNE) // b.ne .target
		fc.addBrPatch(int(l), p)
	...
	case 0x11: // call_indirect
		...
		if _, err := r.ReadU32(); err != nil { // table index (ignored)
			return err
		}
	...
	case 0x21: // local.set
		...
		fc.emitPopR(X9)
		fc.emitStoreLocal64(int(idx), X9)
	...
	case 0x28:
		_, off, err := readMemArg(r); if err != nil { return err }
	...
	case 0x40: // memory.grow — stub: always returns -1
		...
		fc.emitDropN(1)
		fc.Emit32(0x92800009) // movn x9, #0  → x9 = -1
		fc.emitPushR(X9)
	...
	case 0x41: // i32.const
		...
		fc.MOVZ32(X9, uint16(uint32(v)), 0)
		if uint32(v) > 0xFFFF {
			fc.MOVK(X9, uint16(uint32(v)>>16), 1)
		}
		fc.emitPushR(X9)
	...
	case 0x43: // f32.const
		...
		fc.MOVZ32(X9, uint16(v), 0)
		if v > 0xFFFF {
			fc.MOVK(X9, uint16(v>>16), 1)
		}
		fc.emitPushR(X9)
	...
	case 0x44: // f64.const
		...
		fc.emitMov64(X9, binary.LittleEndian.Uint64(raw))
		fc.emitPushR(X9)
	...
	case 0xD2: // ref.func
		...
		// ADRP + ADD to get the (callback trampoline) function address, then push.
		adrpOff := fc.ADRP(X9)
		...
		fc.ADDSI(X9, X9, 0) // lo12 placeholder
		fc.emitPushR(X9)
...
// emitMov64 materialises an arbitrary 64-bit constant into dst using
// MOVZ + up to three MOVK instructions.
func (fc *funcCompiler) emitMov64(dst int, v uint64) {
	fc.MOVZ(dst, uint16(v), 0)
	if v>>16 != 0 {
		fc.MOVK(dst, uint16(v>>16), 1)
	}
	if v>>32 != 0 {
		fc.MOVK(dst, uint16(v>>32), 2)
	}
	if v>>48 != 0 {
		fc.MOVK(dst, uint16(v>>48), 3)
	}
}
